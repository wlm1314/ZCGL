...
public class PullToRefreshNestedScrollView extends PullToRefreshBase<CustomNestedScrollView> {
	...
	@Override
	protected CustomNestedScrollView createRefreshableView(Context context, AttributeSet attrs) {
		CustomNestedScrollView scrollView;
		if (VERSION.SDK_INT >= VERSION_CODES.GINGERBREAD) {
			scrollView = new InternalScrollViewSDK9(context, attrs);
		} else {
			scrollView = new CustomNestedScrollView(context, attrs);
		}
	...
	@TargetApi(9)
	final class InternalScrollViewSDK9 extends CustomNestedScrollView {
	...
	public interface ScrollViewListener {
		void onScrollChanged(PullToRefreshNestedScrollView scrollView, int x, int y,
							 int oldx, int oldy);
	}
